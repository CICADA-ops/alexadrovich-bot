...
async def translation_without_message(message: Message) -> None:
    ...
    await bot.download_file(file_path, f'{message.from_user.full_name}_voice_message.ogg')

    audio = AudioSegment.from_ogg(f'{message.from_user.full_name}_voice_message.ogg')
    audio.export(f'{message.from_user.full_name}_voice_message.wav', format="wav")
...
async def translation(message: Message, state: FSMContext) -> None:
    ...
    audio.export('voice_message.ogg', format="wav")
